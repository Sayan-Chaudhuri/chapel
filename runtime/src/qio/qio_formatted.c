...
{  
  ...
  if(!err)                                     
  {
    ...
    ret[0]='\0';
    //Why file_length+1 instead of file_length? When the program tries to read one more character than 
    //the prescribed file length then, if we have already reached the file end,
    //reading 1 more character will generate EOF. But, if the file length has increased
    //while reading, we have just read one more character.
  ...
  {    
    ...
    while(true)
    {
      ...
      err_reading_file = qio_channel_read(false, ch,&ret[total_amt_read],chunkSize, &amt);   
      total_amt_read+=amt;                                                                
